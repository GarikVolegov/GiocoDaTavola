...
// generic JSON replacer/reviver so it is field-agnostic: any Map or Set on the
// Room (current or future) round-trips automatically; only the Deck class
// instance needs a named case. NEVER sent to clients — secret votes stay server-side.
...
function replacer(_key: string, value: unknown): unknown {
  ...
  if (value instanceof Set) return { __t: 'Set', v: [...value] } satisfies TaggedSet;
...
function reviver(_key: string, value: unknown): unknown {
  if (value && typeof value === 'object' && '__t' in value) {
    ...
    if (tag.__t === 'Set') return new Set((value as TaggedSet).v);
